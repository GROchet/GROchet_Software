...
/*
  ...
  */
        if ((posX > MAX_POS_X) & (digitalRead(BTN_PIN_UP) == LOW)) { deltaA -= speed[difficulty]; deltaB -= speed[difficulty]; }
        if ((digitalRead(LMTSW_X) == LOW) & (digitalRead(BTN_PIN_DOWN) == LOW)) { deltaA += speed[difficulty]; deltaB += speed[difficulty]; }
        if ((digitalRead(LMTSW_Y) == LOW) & (digitalRead(BTN_PIN_LEFT) == LOW)) { deltaA -= speed[difficulty]; deltaB += speed[difficulty]; }
        if ((posY > MAX_POS_Y) & (digitalRead(BTN_PIN_RIGHT) == LOW)) { deltaA += speed[difficulty]; deltaB -= speed[difficulty]; }
